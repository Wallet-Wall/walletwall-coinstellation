...
// env-var name is absent — this is the layer that keeps a fixture-only surface
// from silently regaining hardcoded query ids or provider wiring.
// (Calibrated to produce zero matches on the current fixture-only tree.)
...
describe('No secrets committed', () => {
  ...
  // Scans the RAW listing (not the extension-filtered subset), so a bare
